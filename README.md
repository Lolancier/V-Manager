...
通过 OpenAI 兼容的 function calling，复杂任务模型可主动调用 32 个工具，并按当前意图只注入相关工具集合：
...
`0.7.0` 已于 2026-07-22 完成本阶段开发并进入功能冻结。当前主线以稳定保存现有成果为目标，未完成功能不在本阶段继续扩展。完整状态、验证结果和后续方向见 [阶段状态与后续规划](docs/project-status-0.7.md)，逐项开发记录见 [CHANGELOG](CHANGELOG.md)。
